...
,showOutput as (SELECT a.ACCS_MTHD_KEY ma_tb,
                       ...
                       (SELECT b.ma_to
                        ...
                        WHERE b.ma_nv = a.HRM_USER_DK AND b.thang = 202502) AS ma_to_TTTB,
                       (SELECT b.TEN_TO
                        FROM ttkd_bsc.nhanvien b
                        WHERE b.ma_nv = a.HRM_USER_DK AND b.thang = 202502) AS ten_to_TTTB,
                       (SELECT b.MA_PB
                        FROM ttkd_bsc.nhanvien b
                        WHERE b.ma_nv = a.HRM_USER_DK AND b.thang = 202502) AS ma_pb_TTTB,
                       (SELECT b.TEN_PB
                        FROM ttkd_bsc.nhanvien b
                        WHERE b.ma_nv = a.HRM_USER_DK AND b.thang = 202502) AS ten_pb_TTTB,
                ...
                SELECT b.ACCS_MTHD_KEY,
                      ...
                      CASE
                       ...
                       (SELECT c.ma_to
                        ...
                        WHERE c.ma_nv = b.HRM_CODE AND c.thang = 202502) AS ma_to_bangoi,
                       (SELECT c.TEN_TO
                        FROM ttkd_bsc.nhanvien c
                        WHERE c.ma_nv = b.HRM_CODE AND c.thang = 202502) AS ten_to_bangoi,
                       (SELECT c.MA_PB
                        FROM ttkd_bsc.nhanvien c
                        WHERE c.ma_nv = b.HRM_CODE AND c.thang = 202502) AS ma_pb_bangoi,
                       (SELECT c.TEN_PB
                        FROM ttkd_bsc.nhanvien c
                        WHERE c.ma_nv = b.HRM_CODE AND c.thang = 202502) AS ten_pb_bangoi
...
delete
from TBL_VNP_BRIS where thang = 202502;
...
insert into ttkdhcm_ktnv.TBL_VNP_BRIS(MA_TB, PHANLOAI_NHOM, NGAY_INIT, LOAIKENH_DK_TTTB, MANV_DK_TTTB, TENNV_DK_TTTB, MATO_DK_TTTB, TENTO_DK_TTTB, MAPB_DK_TTTB, TENPB_DK_TTTB, GOICUOC, CHUKY_GOI, DTHU_GOI, ACC_INIT, KENH_BANGOI, ACC_BANGOI, LOAIKENH_DK_BANGOI, MANV_DK_BANGOI, TENNV_DK_BANGOI, MATO_DK_BANGOI, TENTO_DK_BANGOI, MAPB_DK_BANGOI, TENPB_DK_BANGOI, DTHU_HMM, DTHU_TKC, TONG_DTHU_PTM, ACC_GHINHAN, LOAIKENH_GHINHAN, NGUON, NGAY_INS,  DTHU_TKC_NGAY, TONG_DTHU_PTM_NGAY, GEO_STATE_KEY, THANG)
    ...
    where  to_number(TO_CHAR(ngay_init, 'yyyymm')) = 202502;
...
on b.msisdn = a.ma_tb
;
select * from ttkdhcm_ktnv.baocao_doanhthu_dongtien_pktkh  where thang = 202502;
...
where thang = 202502 and chuky_goi is null and ma_tb in (
select ma_tb from x_sb
);
update ttkdhcm_ktnv.TBL_VNP_BRIS
set DTHU_GOI_NOVAT = nvl(round(DTHU_GOI/1.1),0)
where DTHU_GOI_NOVAT =0 and thang = 202502 and ma_tb in (
select ma_tb from x_sb
)
;
