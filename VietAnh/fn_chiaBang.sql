...
BEGIN
    ...
    v_sql := 'CREATE TABLE BL_' || p_thang || '_TONGHOP_1 AS ' ||
    ...
    v_query := 'SELECT ma_kpi, giao, thuchien, tyle_thuchien, mucdo_hoanthanh, diem_cong, diem_tru ' ||
               'FROM ttkd_bsc.blkpi_danhmuc_kpi WHERE thang = ' || p_thang || ' ORDER BY ma_kpi ASC';
    ...
    LOOP
        ...
        IF v_thuchien = 1 THEN
            v_sql := v_sql || 'MAX(CASE WHEN ma_kpi = ''' || v_ma_kpi || ''' THEN THUCHIEN END) AS "' || v_ma_kpi || '_THUCHIEN", ';
        END IF;

        IF v_tyle_thuchien = 1 THEN
            v_sql := v_sql || 'MAX(CASE WHEN ma_kpi = ''' || v_ma_kpi || ''' THEN TYLE_THUCHIEN END) AS "' || v_ma_kpi || '_TYLE_THUCHIEN", ';
        END IF;

        IF v_mucdo_hoanthanh = 1 THEN
            v_sql := v_sql || 'MAX(CASE WHEN ma_kpi = ''' || v_ma_kpi || ''' THEN MUCDO_HOANTHANH END) AS "' || v_ma_kpi || '_MUCDO_HOANTHANH", ';
        END IF;
...
BEGIN
    create_kpi_summary(TO_NUMBER(TO_CHAR(ADD_MONTHS(SYSDATE, -1), 'YYYYMM')));
END;
/
drop table Bl_202408_tonghop_1;
select * from Bl_202408_tonghop_1;
