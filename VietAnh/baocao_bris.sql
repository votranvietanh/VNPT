...
--PL1_MONTHLY,
--PL4_MONTHLY,
delete from PL1_2024 where thang = 202408;
delete from PL4_2024 where thang = 202408;
delete from PL1_MONTHLY where thang = 202408;
delete from PL4_MONTHLY where thang = 202408;
delete from PL5 where thang = 202408;

select * from PL1_2024 where thang = 202408;
select * from PL4_2024 where thang = 202408;
select * from PL1_MONTHLY where thang = 202408;
select * from PL4_MONTHLY where thang = 202408;
...
UPDATE PL1_2024
SET thang = 202408
WHERE To_number(TO_CHAR(ngay_kich_hoat, 'yyyymm')) = 202408;
UPDATE PL4_2024
SET thang = 202408
WHERE To_number(TO_CHAR(ngay_kich_hoat, 'yyyymm')) = 202408;
...
---- UPDATE THANG
            UPDATE PL1_MONTHLY
            SET thang = 202408
            WHERE  TO_NUMBER(TO_CHAR(ngay_init, 'yyyymm')) = 202408;
;
            UPDATE PL4_MONTHLY
            SET thang = 202408
            WHERE  TO_NUMBER(TO_CHAR(ngay_init, 'yyyymm')) = 202408;
...
SET 
    a.MATO_DK_TTTB = (SELECT b.MA_TO FROM ttkd_bsc.nhanvien b WHERE b.MA_NV = a.MANV_DK_TTTB AND b.thang = 202408),
    a.TENTO_DK_TTTB = (SELECT b.TEN_TO FROM ttkd_bsc.nhanvien b WHERE b.MA_NV = a.MANV_DK_TTTB AND b.thang = 202408),
    a.MAPB_DK_TTTB = (SELECT b.MA_PB FROM ttkd_bsc.nhanvien b WHERE b.MA_NV = a.MANV_DK_TTTB AND b.thang = 202408),
    a.TENPB_DK_TTTB = (SELECT b.TEN_PB FROM ttkd_bsc.nhanvien b WHERE b.MA_NV = a.MANV_DK_TTTB AND b.thang = 202408),
    a.TENNV_DK_TTTB = (SELECT b.ten_nv FROM ttkd_bsc.nhanvien b WHERE b.MA_NV = a.MANV_DK_TTTB AND b.thang = 202408)
WHERE to_number(TO_CHAR(a.ngay_init, 'yyyymm')) = 202408
AND EXISTS (SELECT 1 FROM ttkd_bsc.nhanvien b WHERE b.MA_NV = a.MANV_DK_TTTB AND b.thang = 202408);

-- C?p nh?t các tr??ng MATO_DK_BANGOI, TENTO_DK_BANGOI, MAPB_DK_BANGOI, TENPB_DK_BANGOI, TENNV_DK_BANGOI trong b?ng PL1_MONTHLY
UPDATE PL1_MONTHLY a
SET 
    a.MATO_DK_BANGOI = (SELECT b.MA_TO FROM ttkd_bsc.nhanvien b WHERE b.MA_NV = a.MANV_DK_BANGOI AND b.thang = 202408),
    a.TENTO_DK_BANGOI = (SELECT b.TEN_TO FROM ttkd_bsc.nhanvien b WHERE b.MA_NV = a.MANV_DK_BANGOI AND b.thang = 202408),
    a.MAPB_DK_BANGOI = (SELECT b.MA_PB FROM ttkd_bsc.nhanvien b WHERE b.MA_NV = a.MANV_DK_BANGOI AND b.thang = 202408),
    a.TENPB_DK_BANGOI = (SELECT b.TEN_PB FROM ttkd_bsc.nhanvien b WHERE b.MA_NV = a.MANV_DK_BANGOI AND b.thang = 202408),
    a.TENNV_DK_BANGOI = (SELECT b.ten_nv FROM ttkd_bsc.nhanvien b WHERE b.MA_NV = a.MANV_DK_BANGOI AND b.thang = 202408)
WHERE to_number(TO_CHAR(a.ngay_init, 'yyyymm')) = 202408
AND EXISTS (SELECT 1 FROM ttkd_bsc.nhanvien b WHERE b.MA_NV = a.MANV_DK_BANGOI AND b.thang = 202408);
...
SET 
    a.MATO_DK_TTTB = (
        ...
        WHERE (a.MANV_DK_TTTB = b.MA_NV AND b.thang = 202408)
    ),
    a.TENTO_DK_TTTB = (
        SELECT b.TEN_TO 
        FROM ttkd_bsc.nhanvien b 
    ...
    a.MATO_DK_BANGOI = (
        SELECT b.MA_TO 
        FROM ttkd_bsc.nhanvien b 
        WHERE (a.MANV_DK_BANGOI = b.MA_NV AND b.thang = 202408)
    ),
    a.TENTO_DK_BANGOI = (
        SELECT b.TEN_TO 
        FROM ttkd_bsc.nhanvien b 
...
WHERE 
    TO_NUMBER(TO_CHAR(a.ngay_init, 'yyyymm')) = 202408
    AND EXISTS (
        SELECT 1 
        FROM ttkd_bsc.nhanvien b 
...
---- insert to view:
        ...
        select * from PL5 where thang = 202408;
...
--            upgrade
    ...
    ---- phanloai_nhomm:
        ...
        where thang = 202408 --(SYSDATE, 'DD/MM/YYYY')
        ;
        select * from pl1_monthly where thang = 202408;
        --
              MERGE INTO PL5 a
            ...
            ON (
                a.ma_tb = b.ma_tb 
                AND a.ngay_init >= TO_DATE('01/08/2024', 'DD/MM/YYYY') 
        ...
        SET phanloai_nhom = CASE 
            ...
            ELSE phanloai_nhom 
        END
        where thang = 202408--TO_CHAR(SYSDATE, 'DD/MM/YYYY')
        ;
        ...
        SET phanloai_nhom = CASE 
            ...
            ELSE phanloai_nhom 
        END
        where PHANLOAI_NHOM is null and  to_number(TO_CHAR(ngay_init, 'yyyymm')) = 202408--TO_CHAR(SYSDATE, 'DD/MM/YYYY')
        ;
    ---- final:
    --delete xong them :
    delete from ttkdhcm_ktnv.TBL_VNP_BRIS where thang = 202408;
    
    SELECT * FROM ttkdhcm_ktnv.TBL_VNP_BRIS WHERE THANG = 202408;
    select * from PL5 where thang = 202408;
    ...
    where thang = 202408
        ...
        from ttkdhcm_ktnv.TBL_VNP_BRIS 
        where thang = 202408 group by ma_tb having count(ma_tb)>1);
        ...
        -- del case trung ma_tb vi` nhieu goi_cuoc ->> chose the greatest one
               DELETE FROM ttkdhcm_ktnv.TBL_VNP_BRIS
                ...
                select * from PL1_monthly where thang = 202406;
                select * from PL4_monthly where thang = 202406;
               select* from PL5 where thang = 202408;
