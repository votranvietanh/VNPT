...
select count(*) from manpn.bscc_ptm_bris_P01_moi  where thang = 202412; --bscc_import_ptm_bris_pl01_moi
select * from manpn.bscc_import_ptm_smrs where thang = 202412;
select * from manpn.BUNDLE_XUATKHO_PDH where thang_KH = 202412;
select count(*) from manpn.BSCC_DIGI_DONLE where thang = 202412;
select * from manpn.BSCC_DIGI where thang = 202412;
select count(*) from MANPN.BSCC_IMPORT_BUNDLE_SMRS_CT where thang = 202412;
select count(*) from MANPN.BSCC_IMPORT_BUNDLE_SMRS where thang = 202412;
select count(*) from manpn.bscc_import_goi_bris_p04 where thang = 202412;
select * FROM manpn.bscc_import_kenh_noibo where thang = 202412 ;
select count(*) from manpn.BSCC_IMPORT_CHUOI_TOAN_QUOC where thang = 202412;
select count(*) from manpn.kpi_kenhban_ngoai_noibo@ttkddbbk2 where thang = 202412;
...
----Phân rã đơn giá:
select THANG_PTM, MA_TB, NGAY_KH, USERNAME_KH , ''nv_thay_the from SSS_dgia_202412  where MANV_DKTT ='halv_hcm' and tenkieu_ld='ptm';



select * from ttkd_bsc.ds_diemban_31import where ma_diem_ban in ('HCM1652987KDDDTTGDDBPT') and thang = 202412; --20256
delete from SSS_DGIA_202412; where TENKIEU_LD ='ptm-goi';
----insert PTM:
--        insert into SSS_dgia_202412(MA_TB, nguon, username_kh, thang_ptm, MANV_DKTT, tenkieu_ld, ngay_kh)
-- with cte as(
...
--     ),
--     NVVV AS(
--     SELECT UPPER(SUBSTR(MAIL_VNPT, 1, INSTR(MAIL_VNPT, '@')-1)) AS MAIL, C.* FROM (select * from ttkd_bsc.nhanvien where thang = 202412) C
--     WHERE THANG = 202412
--     )
...
--     left join NVVV C ON b.MA_Nv = C.MAIL AND C.THANG = 202412
...
;
create table SSS_dgia_202412 as
select * from SSS_dgia_202411 where 1=0
;

 insert into SSS_dgia_202412(nguon,ma_tb,username_kh,manv_dktt,tenkieu_ld,thang_ptm,ngay_kh)
...
-----------
    ...
    ;select 'bris',so_tb,nvl(MA_CUA_USER_TIEP_THI_TB,USER_ELOAD_DK_TTTB),nvl(MA_CUA_USER_TIEP_THI_TB,USER_ELOAD_DK_TTTB),'ptm',thang, NGAY_KICH_HOAT
        from manpn.bscc_ptm_bris_P01_moi
            where thang = 202412
            and  so_tb not in (select ma_tb from SSS_dgia_202412 where tenkieu_ld = 'ptm')
            ;
    -----------

---------
    insert into SSS_dgia_202412(nguon,ma_tb,username_kh,tenkieu_ld,thang_ptm,ngay_kh)
    select 'smrs','84' || SUBSCRIBER_ID a, ACCOUNT_DK, 'ptm', THANG, to_date(NGAY_KH, 'yyyy-mm-dd')
    from manpn.bscc_import_ptm_smrs
    where thang = 202412
        and  '84' || SUBSCRIBER_ID not in (select ma_tb from SSS_dgia_202412 where tenkieu_ld = 'ptm');

    ------------
           MERGE INTO SSS_dgia_202412 a
USING (
    SELECT ma_tb, ma_hrm
    FROM (
        ...
        FROM (select * from manpn.bscc_import_ptm_shopctv a  where thang = 202412) a
...
WHEN MATCHED THEN
    UPDATE SET a.username_kh = x.ma_hrm
      ;
-- T10 ko tinh
--     insert into SSS_dgia_202412(nguon,ma_tb,username_kh,manv_ptm,tenkieu_ld,thang_ptm,ngay_kh)
--     select 'luongtinh',somay,nguoi_gt,manv_ptm,'ptm',TO_NUMBER(TO_CHAR((ngay_ld), 'YYYYMM')),ngay_ld
--     from TTKD_BSC.DT_PTM_VNP_202412
...
----insert gói PTM:
    ...
    SELECT 'bundle_xuatkho', B.MA_TB, ACCOUNT_DK, LOAI_SIM,'ptm-goi', GIA_GOI, THANG_KH, MA_NV, MA_PB
        ,(select ngay_kh from SSS_dgia_202412 where tenkieu_Ld = 'ptm' and ma_tb=b.ma_tb)
    FROM (
            SELECT B.*, ROW_NUMBER() OVER (PARTITION BY ma_tb ORDER BY ngay_CN DESC) AS row_num
            FROM manpn.BUNDLE_XUATKHO_PDH B
             WHERE B.THANG_KH = 202412
        ) B
       where
           B.row_num = 1
          and B.MA_TB IN (SELECT A.MA_TB FROM SSS_dgia_202412 A WHERE tenkieu_ld='ptm')
    ;
    ----IMPORT gói c?a kênh Digishop tr??c: https://digishop.vnpt.vn/digitalShop/stats/statsMobileDetail // ct sim so
    INSERT INTO SSS_dgia_202412 (thang_ptm,nguon, tenkieu_ld,  MA_TB, tien_GOI, ten_goi, username_kh, CK_GOI, ngay_kh,thang_bd)
    ...
    AND STB IN (SELECT MA_TB FROM SSS_dgia_202412 WHERE tenkieu_ld = 'ptm')
    AND STB  NOT IN (SELECT MA_TB FROM SSS_dgia_202412 WHERE tenkieu_ld = 'ptm-goi');
    ...
    WHERE THANG = 202412 and ngay_hoan_thanh is not null -- tại 1 tb có nhiều dòng gói giống nhau, nhưng chỉ có 1 dòng có ngay_ht là dky thành công
    AND STB  IN (SELECT MA_TB FROM SSS_dgia_202412 WHERE tenkieu_ld = 'ptm')
    AND STB  NOT IN (SELECT MA_TB FROM SSS_dgia_202412 WHERE tenkieu_ld = 'ptm_goi');

    -----
    INSERT INTO SSS_dgia_202412  (thang_ptm,tenkieu_ld, nguon, MA_TB, username_kh, ten_goi, tien_goi,  ngay_kh,thang_bd)
    ...
    AND B.MSISDN IN (SELECT A.MA_TB FROM SSS_dgia_202412  A WHERE tenkieu_ld='ptm')
        AND B.MSISDN NOT IN (SELECT MA_TB FROM SSS_dgia_202412 A WHERE tenkieu_ld='ptm-goi')
        ;
        --SMRS-bundle:
    INSERT INTO SSS_dgia_202412 (tenkieu_ld, nguon, MA_TB, username_kh, ten_goi, tien_goi, THANG_ptm, ngay_kh,thang_bd) -- ko co chu -ky trong file
    ...
    WHERE THANG = 202412
    AND '84'||B.SUBSCRIBER_ID IN (SELECT A.MA_TB FROM SSS_dgia_202412 A WHERE tenkieu_ld='ptm')
    AND '84'||B.SUBSCRIBER_ID NOT IN (SELECT MA_TB FROM SSS_dgia_202412 A WHERE tenkieu_ld='ptm-goi');

    --bris
   INSERT INTO SSS_dgia_202412  ( thang_ptm,nguon, MA_TB, ten_goi, CK_GOI, tien_goi, -- bỏ cột loại _kênh
...
--//thang 12 da test (KQ:    )-thang 12 chay code inss bris tren xem co bnhiu rec dc add, roi so sanh voi code nay
    INSERT INTO SSS_dgia_202412 (thang_ptm, nguon, MA_TB, ten_goi, CK_GOI, tien_goi,
                           username_kh, MANV_PTM, tenkieu_ld, ngay_kh)
;
with tmp_data as (SELECT thang,
                  ...
                  LEFT JOIN SSS_dgia_202412 x1
                  ON b.ACCS_MTHD_KEY = x1.MA_TB
--
                  WHERE b.LOAIHINH_TB = 'TT'
                    AND b.LOAI_TB_THANG = 'PTM'
                    AND b.THANG = 202412
                    AND x1.MA_TB IS NOT NULL -- Chỉ lấy các bản ghi có trong bảng `x_code_202412` với `tenkieu_ld = 'ptm'`
...
----- END IMPORT ------
--
     delete from SSS_dgia_202412 where( tenkieu_ld ='ptm-goi' and CK_GOI_TLDG = 0) or nguon in( 'selfcare','myvnpt')
...
--- Cho trường hợp các phòng bán hàng gán manv mới,
    update SSS_dgia_202412
    set manv_dktt = username_kh
    where manv_dktt is null;
     --tạo index cho lẹ xog xóa chứ udpate lâu quá :((
     drop index idx_tmp;
    create index idx_tmp on SSS_dgia_202412(ma_tb,username_kh);
    ;
        update SSS_dgia_202412 a
        set MANV_PTM = (select (b.ma_nv) from ttkd_bsc.nhanvien b where b.thang = 202412 and b.USER_CCBS = a.username_kh) -- tháng 9 ttvt_ctv086203_hcm đag bị double nên dùng max
        where manv_ptm is null
        ;
        update SSS_dgia_202412 a
        set MANV_PTM = (select b.ma_nv from ttkd_bsc.nhanvien b where b.thang = 202412 and b.USER_CCOS = a.username_kh)
        where manv_ptm is null
        ;
        update SSS_dgia_202412 a
        ...
        --TH appemployee bi loi~ ko ghi nhan goi cho nv thuc hien toan trinh
            update SSS_dgia_202412 a
            set (username_kh,MANV_DKTT,manv_ptm) = (select x.manv_ptm,x.manv_ptm,x.manv_ptm from  SSS_dgia_202412 x where x.ma_tb =a.ma_tb and x.TENKIEU_LD='ptm'  )
            where TENKIEU_LD = 'ptm-goi'
...
select * from ttkd_bsc.ds_diemban_31import;
        ...
        ON (a.username_kh = d.ma_diem_ban AND d.thang = 202412)
        WHEN MATCHED THEN
            UPDATE SET a.manv_ptm = d.manv_hrm
            WHERE a.username_kh in (select (ma_diem_ban) from ttkd_bsc.ds_diemban_31import where thang = 202412);


            MERGE INTO SSS_dgia_202412 a
        USING ttkd_bsc.ds_diemban_31import d
        ON (a.username_kh = to_char(d.so_eload) AND d.thang = 202412)
        WHEN MATCHED THEN
            UPDATE SET a.manv_ptm = d.manv_hrm
            WHERE a.username_kh in (select to_char(so_eload) from ttkd_bsc.ds_diemban_31import where thang = 202412)
...
;
        ...
        SET MANV_PTM = (
            ...
            WHERE manv_dktt = b.ma_nv and thang = 202412
        )
...
---===UPDATE PHAN_LOAI_KENH:
            ...
            --note 14/11: edit thanh merge di chu update waste 7p,9p lun
                   MERGE INTO SSS_dgia_202412 a
                    ...
                    ON (a.ma_tb = b.ma_tb AND a.THANG_PTM = 202412)
                    WHEN MATCHED THEN
...
-- gói bundle xuất kho
        update SSS_dgia_202412 a
          ...
          where tien_goi is null and BUNDLE_XK = 1 --x.GIA_GOI_CO_VAT*(1-CHIET_KHAU)
                    ;
select * from  SSS_DGIA_202412 where MANV_DKTT like '%LK%'  AND TENKIEU_LD ='ptm';
...
select * from one_line_202412 where ma_tb  in ('84814005472','84816240746','84814387588');
select * from SSS_dgia_202412_2 where ma_tb ='84916374837';
select * from SSS_dgia_202412 where ma_tb ='84813461192';
...
----=== đơn giá THỦ ĐỨC
select * from one_line_202412 where ma_tb ='84886703462';
select * from SSS_dgia_202412 where ma_tb ='84886703462';
select * from khieunai_td_dgia;
update SSS_dgia_202412 a
set MANV_PTM = (select x.NGUOI_THAYTHE x from khieunai_td_dgia x
                                            where x.thang = 202412
                                               and x.MA_TB = a.ma_tb
                                               and x.ma_tiepthi = username_kh)
where  a.ma_tb in (select ma_tb from khieunai_td_dgia x where x.thang = 202412)
    and TENKIEU_LD ='ptm'
;
update SSS_dgia_202412 a
    ...
    where ma_tb in  (select ma_tb from khieunai_td_dgia where thang = 202412) and TENKIEU_LD ='ptm'
      ;


update SSS_dgia_202412_2 a
set MANV_PTM = (select x.NGUOI_THAYTHE x from khieunai_td_dgia x
                                            where x.thang = 202412
                                               and x.MA_TB = a.ma_tb
                                               and x.ma_tiepthi = username_kh)
...
where  a.ma_tb in (select ma_tb from khieunai_td_dgia x where x.thang = 202412);
 update one_line_202412 a
    ...
    where ma_tb in  (select ma_tb from khieunai_td_dgia where thang = 202412)
      ;
;      drop index idx_tmp;
...
select * from SSS_dgia_202412_2 ;
drop table SSS_dgia_202412_2;
create table SSS_dgia_202412_2 as -- tạo bảng 2 dòng ( ptm và chỉ 1 gói)
...
;
delete from SSS_dgia_202412_2 where ten_goi = 'Sim D_VinaXtra' ;


-- drop table one_line_202412;
drop table one_line_202412;
CREATE TABLE one_line_202412 AS
  (SELECT thang_ptm,
 ...
 FROM SSS_DGIA_202412_2
 GROUP BY ma_tb, thang_ptm, dthu_KPI);
GRANT UPDATE ON vietanhvh.one_line_202412 TO MANPN;

----update xử mục 2 a mẫn
update one_line_202412
...
--tìm nhan vien đat chỉ tieu KK:
            select manv_goi,ten_goi from one_line_202412 where manv_goi in
              (SELECT ma_nv
            FROM ttkd_bsc.dinhmuc_giao_dthu_ptm
            WHERE thang = 202412 and dinhmuc_2 IN (32000000, 30000000)
...
select manv_goi from one_line_202412 where heso_kk = 0.05 --check coi đủ SL NV ở query select trên
group by manv_goi;
...
;
 select * from (
         ...
         from vietanhvh.one_line_202412 a
...
--         MANV_GOI, MATO_GOI, MAPB_GOI, TIEN_GOI, DTHU_DONGIA_GOI, TIEN_THULAO_GOI,
         ...
         from vietanhvh.one_line_202412
union all
 select THANG_PTM thang, NGUON, PHAN_LOAI_KENH, MA_TB, TEN_GOI, CK_GOI_TLDG,
             MANV_GOI ma_nv,b.ten_nv, MATO_GOI ma_to,b.ten_to, MAPB_GOI ma_pb,b.ten_pb,'mua_goi' tenkieu_ld, TIEN_GOI, DTHU_DONGIA_GOI, TIEN_THULAO_GOI,
             DTHU_KPI, LYDO_KHONGTINH
...
;
 update one_line_202412 a
    ...
    where manv_goc ='VNP016957'
       ;
select * from one_line_202412
where ma_tb ='84915942378'
;
--cap nhat manv goi cho chij thang ch xong
select * from MANPN.manpn_goi_tonghop_202412
where kenh_ban like '%luongtinh%'
;
select * from manpn.manpn_GOI_TONGHOP_202412 a where a.ma_tb in( select ma_tb from one_line_202412 where heso_hhbg >0 and TIEN_THULAO_GOI =0);
         select * from one_line_202412 where MA_TB ='84812216048';
SELECT *from one_line_202412 WHERE NGUON LIKE '%Đơn giá%'; --469075475 469570975
select * from one_line_202412 where ma_tb ='84812068195';


select * from manpn.manpn_goi_tonghop_202412 where ma_tb in (select ma_tb from dgia_tndn_simso where thang = 202412 ) and loai_tb ='ptm' ;

select * from one_line_202412 where ma_tb  in (select ma_tb from dgia_tndn_simso where thang = 202412 ) and tien_thulao_dnhm =0;

select *from ttkd_bsc.tonghop_ct_dongia_ptm where thang = 202412 and ma_tb ='84812054201';
