...
update ob_ckd_ct
            set thang = 202501
            where thang is null;
select* from ob_ckd_ct where thang = 202501;




with raw_dt as (
    select * from manpn.bscc_import_goi_bris_p04 where LOAI_TB_THANG ='HH' and thang = 202501 and accs_mthd_key = '84912169815'
)
select * from raw_dt
...
select *from ttkdhcm_ktnv.vnp_yeucau where sothuebao ='833577251';
    where to_char(NGAYHOAMANG,'yyyymm') =202501;
...
select * from DONGIA_DTHU_HIENHUU_202501 where ma_tb ='84948966139';
...
drop table DONGIA_DTHU_HIENHUU_202501;
select * from DONGIA_DTHU_HIENHUU_202501 ;
select * from DONGIA_DTHU_HIENHUU_202501 where loai_gd ='DANGKY' and ma_tb in (select ma_tb
...
drop table DONGIA_DTHU_HIENHUU_202501;
---START
CREATE TABLE DONGIA_DTHU_HIENHUU_202501 AS
...
where LOAI_TB_THANG ='HH' and thang = 202501
...
--t11: 84903941945
MERGE INTO DONGIA_DTHU_HIENHUU_202501 tgt
USING (
    SELECT
        t1.ma_tb,
        t1.ngay_kh,
        t2.ngay_kh AS ngay_kh_muon
    FROM DONGIA_DTHU_HIENHUU_202501 t1
    JOIN DONGIA_DTHU_HIENHUU_202501 t2
...
INSERT INTO DONGIA_DTHU_HIENHUU_202501
     select ACCS_MTHD_KEY ma_tb
             ,  CASE
                    WHEN (TO_DATE(ACTVTN_DT,'DD/MM/YYYY HH24:MI:SS') < ADD_MONTHS(trunc(sysdate, 'mm'), -1) AND LOAIHINH_TB = 'TT') --1
                      OR (TO_DATE(ACTVTN_DT,'DD/MM/YYYY HH24:MI:SS') < ADD_MONTHS(trunc(sysdate, 'mm'), -6) AND LOAIHINH_TB = 'TS') --6
...
delete DONGIA_DTHU_HIENHUU_202501
where
...
--                      where thang = 202501
                    union all
                     ...
                     from ob_hvc2_ct
                     where DTHU_GOI> 20000 and thang = 202501
                     )

                 WHERE TRUNC(ngay_kh) = TRUNC(DONGIA_DTHU_HIENHUU_202501.ngay_kh)
               )
 ;

---HVC+BANGOI: sua bang tieudung
insert into DONGIA_DTHU_HIENHUU_202501 ( MA_TB, USER_BAN_GOI
                                , MA_HRM
                                , TEN_GOI
                                , DTHU_GOI
                                , NGAY_KH
...
Select a.*
    ...
    ,'DANGKY' loai_gd
From data a
Left join  CUOCVINA.tieudung_bts_202501@ttkddbbk2 b
    on a.ma_tb = b.subscriber_id_84
;
delete
from DONGIA_DTHU_HIENHUU_202501 where nguon ='HVC_moi';
insert into DONGIA_DTHU_HIENHUU_202501 ( MA_TB, USER_BAN_GOI
                                , MA_HRM
                                , TEN_GOI
                                , DTHU_GOI
...
select '84'||ma_tb,USER_DK,MA_NV,TEN_GOI,DTHU_GOI,TO_DATE(REPLACE(NGAY_SD, '.0', ''), 'yyyy-mm-dd hh24:mi:ss'),'HVC'
 ,CASE
    ...
    ,(select x.LOAIHINH_TB from manpn.bscc_import_goi_bris_p04 x where x.thang = 202501 and x.ACCS_MTHD_KEY = '84'||a.ma_tb and trunc(TO_DATE(REPLACE(a.NGAY_SD, '.0', ''), 'yyyy-mm-dd hh24:mi:ss')) = trunc(x.REGIS_DT) group by x.LOAIHINH_TB)
,DTHU_GOI,'HVC_moi',
...
--                   when TEN_GOI like '%_0K' and DOANHTHU_TRUOC_GIAHAN = 0 then 'GIAHAN'
                when  DTHU_GOI > DTHU_TRUOCOB then 'NANG_CHUKY'
                when DTHU_GOI <= DTHU_TRUOCOB then 'GIAHAN' -- test
                else 'GIAHAN' end as loai_gd
from OB_HVC2_CT a
Left join  CUOCVINA.tieudung_bts_202412@ttkddbbk2 b -- edit để tìm ngày_kh cho sim trong file bangoi ccos
    on a.ma_tb = b.subscriber_id
where a.DTHU_GOI>24000 and a.thang = 202501
;
...
where
 ...
 ;
delete from DONGIA_DTHU_HIENHUU_202501 where nguon ='CKN_CKD';
select * from DONGIA_DTHU_HIENHUU_202501 where ma_tb ='84836158567';
--OK// INSERT tập gia hạn CKN -CKD
INSERT INTO DONGIA_DTHU_HIENHUU_202501(LOAI_GD,IS_TBHH,ma_tb,USER_BAN_GOI,thang_kh_sim,ngay_kh,ten_goi,chu_ky,dthu_goi,DTHU_TLDG,LOAIHINH_TB,LOAI_HVC,nguon)
...
SELECT a.LOAI_GD, CASE
    ...
    END AS CHU_KY_GOI2,
     ...
     , case when a.ma_tb in (select ma_tb  from ob_hvc_ct where thang = 202501) then 'HCV'
...
--update HVC
            MERGE INTO DONGIA_DTHU_HIENHUU_202501 a
        USING (
            SELECT ACCS_MTHD_KEY
            FROM (select ACCS_MTHD_KEY from  bi_nctt.hvc_202501_chot@coevnpt where tinh_PSC = 'HCM')
            ) b
...
--         WHERE a.nguon IN ('CKN_CKD', 'OB_BG')
        ;
        UPDATE DONGIA_DTHU_HIENHUU_202501
        SET loai_hvc = 'NOHVC'
...
WITH src_data AS (
    SELECT 202501 AS thang,
           ...
           x.ma_vtcv
           ,(select ma_to from ttkd_bsc.nhanvien where ma_nv=ma_hrm and thang = 202501) ma_to
           ,(select ten_to from ttkd_bsc.nhanvien where ma_nv=ma_hrm and thang = 202501) ten_to
...
--            ROW_NUMBER() OVER (PARTITION BY a.ma_tb ORDER BY a.USER_BAN_GOI, a.dthu_goi DESC) AS rnk
    ...
    LEFT JOIN ttkd_bsc.nhanvien x
           ON a.ma_hrm = x.ma_nv AND x.thang = 202501
)
-- ds stb loai_tru
, prevent_data AS (
    SELECT somay
    FROM khanhtdt_ttkd.CCBS_ID1896@TTKDDB
    WHERE thang = 202501 AND kieu_ld = 'P -> C'
)
SELECT a.*,
       CASE
           WHEN IS_TBHH = 0
...
select * from S_DONGIA_DTHU_HIENHUU_202501_test where ma_tb ='84888176039';
    --ADMIN kich giup: user GDV không được quyền tác động bán gói:
create index inx_dg_202501 on S_DONGIA_DTHU_HIENHUU_202501_test(ma_tb);
-- drop index inx_dg;
MERGE INTO S_DONGIA_DTHU_HIENHUU_202501_test b
USING (
    SELECT
        LOAIHINH_TB,
        ma_tb,
    ...
    FROM (
        ...
        JOIN
            S_DONGIA_DTHU_HIENHUU_202501_test b ON '84' || a.sothuebao = b.ma_tb
        LEFT JOIN
            ttkd_bsc.nhanvien x ON x.thang = 202501 AND a.userxuly = x.nhanvien_id
        WHERE
...
update S_DONGIA_DTHU_HIENHUU_202501_test a
...
--update ma_hrm con thieu:
        update S_DONGIA_DTHU_HIENHUU_202501_test a
        set ma_hrm = (select (b.ma_nv) from ttkd_bsc.nhanvien b where b.thang = 202501 and b.ma_nv = a.USER_BAN_GOI) -- tháng 9 ttvt_ctv086203_hcm đag bị double nên dùng max
        where USER_BAN_GOI like 'CTV%'
            or  USER_BAN_GOI like 'VNP%'
        ;
      update S_DONGIA_DTHU_HIENHUU_202501_test a
        set ma_hrm = (select (b.ma_nv) from ttkd_bsc.nhanvien b where b.thang = 202501 and b.USER_CCBS = a.USER_BAN_GOI) -- tháng 9 ttvt_ctv086203_hcm đag bị double nên dùng max
        where ma_hrm is null

        ;
        update S_DONGIA_DTHU_HIENHUU_202501_test a
        set ma_hrm = (select b.ma_nv from ttkd_bsc.nhanvien b where b.thang = 202501 and b.USER_CCOS = a.USER_BAN_GOI)
        where ma_hrm is null
        ;
        update S_DONGIA_DTHU_HIENHUU_202501_test a
...
update S_DONGIA_DTHU_HIENHUU_202501_test
    set (ten_nv,TEN_PB,ma_to,ten_to, MA_VTCV, MA_PB) = (select x.ten_nv,x.TEN_PB,x.ma_to,x.ten_to ,x.MA_VTCV, x.MA_PB from ttkd_bsc.nhanvien x where ma_hrm =ma_nv and thang = 202501)
;
update S_DONGIA_DTHU_HIENHUU_202501_test
...
WHERE a.ma_tb IN (
    SELECT a.ma_tb
    FROM S_DONGIA_DTHU_HIENHUU_202501_test a
    JOIN S_DONGIA_DTHU_HIENHUU_202501_test b
      ON a.ma_tb = b.ma_tb
...
delete
from
S_DONGIA_DTHU_HIENHUU_202501_test
where ma_tb in (select a.ma_tb from S_DONGIA_DTHU_HIENHUU_202501_test a
   , S_DONGIA_DTHU_HIENHUU_202501_test b
...
;
select *
from S_DONGIA_DTHU_HIENHUU_202501_test where ma_tb ='84886000900';
...
select THANG, MA_TB, TRANGTHAI, THANG_DKTT_TB, TEN_GOI, DTHU_GOI, NGAY_KH_GOI, LOAI_GD, LOAIHINH_TB, USER_BAN_GOI, USER_NAME, LOAI_KENH, MA_NV, TEN_NV, MA_VTCV, MA_TO, TEN_TO, MA_PB, TEN_PB, CONG_CU_BAN_GOI, LOAI_HVC, HESO, DTHU_TLDG, DTHU_KPI, TIEN_THULAO, NGUON, LYDO_KHONGTINH, 'VB384' from dongia_dthh where thang = 202501
;
delete
...
;
select *
from DONGIA_DTHH where thang = 202501 and loai_kenh ='Kênh điểm bán';
select *from ttkd_bsc.va_ct_bsc_vnphh where thang = 202501 and ma_tb in ('84914220248','84917067039','84914016078');;
delete from ttkd_bsc.va_ct_bsc_vnphh where thang = 202501;
insert into ttkd_bsc.va_ct_bsc_vnphh
select * from DONGIA_DTHH where thang = 202501;
...
select count(*) from S_DONGIA_DTHU_HIENHUU_202501_test a; where ma_tb ='84329805606';
select *
from DONGIA_DTHU_HIENHUU_202501 where ma_tb ='84329805606';
...
5/12: da fix rank o nhanvien yeucau kich gium
  ...
  -> update tu code nay
       select * from S_DONGIA_DTHU_HIENHUU_202501_test where ma_tb in (
   select a.ma_tb from S_DONGIA_DTHU_HIENHUU_202501_test a
   , S_DONGIA_DTHU_HIENHUU_202501_test b
   where a.ten_goi =b.ten_goi and  b.NGAY_KH < a.NGAY_KH
  ...
  => del    select * from S_DONGIA_DTHU_HIENHUU_202501_test where ma_tb in (
   select a.ma_tb from S_DONGIA_DTHU_HIENHUU_202501_test a
   , S_DONGIA_DTHU_HIENHUU_202501_test b
...
and  thang = 202501  and loai_gd ='DANGKY' and ma_pb ='VNP0703000'
;

select * from dongia_dthh where ma_tb in ('84914220248','84917067039','84914016078') and thang = 202501;
update dongia_dthh a
set (TEN_NV, MA_VTCV, MA_TO, TEN_TO, MA_PB, TEN_PB) = (select x.TEN_NV, x.MA_VTCV, x.MA_TO, x.TEN_TO, x.MA_PB, x.TEN_PB from ttkd_bsc.nhanvien x  where x.thang = 202501 and a.ma_nv=x.ma_nv)
where thang = 202501 and ma_tb in ('84914220248','84917067039','84914016078');

update S_DONGIA_DTHU_HIENHUU_202501_test
set is_tbhh = CASE
                    WHEN (THANG_KH_SIM < to_number(to_char(ADD_MONTHS(trunc(sysdate, 'mm'), -1),'yyyymm')) AND LOAIHINH_TB = 'TT') --1
                      OR (THANG_KH_SIM < to_number(to_char(ADD_MONTHS(trunc(sysdate, 'mm'), -6),'yyyymm')) AND LOAIHINH_TB = 'TS') --6
                    THEN 1
                    ELSE 0
                END
...
FROM S_DONGIA_DTHU_HIENHUU_202501_test_2
